...
> 状态：待评审  
> 更新日期：2026-08-01  
> 依据：`docs/inital-workflow/character/PM.md`、仓库根目录前端源码 `src/`（交互设计基线，原 `front-baseline/` 已并入仓库根目录）

## 1. 文档目的

本文档定义 Searchis V1 的产品边界、业务规则、数据模型、状态机、验收标准与交付任务。研发、测试与 AI Agent 应以本文档作为 V1 行为基线；仓库根目录前端源码 `src/`（原 `front-baseline/`）负责提供界面布局与交互参考。当二者冲突时，以本文档中带编号的需求和验收标准为准，并在开发前提交差异记录。
...
| A-09 | 不提供任何操作音效、音效设置或音频资源 | 现有前端源码中的音效控件和配置必须删除 | 已确认 |
...
| A-13 | 保留仓库根目录前端基线（原 `front-baseline/`）中定义的 6 步首次使用向导 | 向导按 Arch Linux/KDE/X11 能力重新实现 | 已确认 |
...
- 与仓库根目录前端基线（原 `front-baseline/`）对应的浅色、深色、跟随系统主题。
...
6. 前端基线（原 `front-baseline/`）与产品实现的差异已形成清单并经产品确认。
...
- [ ] **T-411** 从仓库根目录前端删除全部操作音效控件、配置字段和音频资源。
...
- [ ] **T-608** 建立前端基线（原 `front-baseline/`）差异清单并完成产品验收。
