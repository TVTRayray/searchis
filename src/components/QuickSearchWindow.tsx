...
import { AlertCircle, LayoutGrid, Lock, Star } from "lucide-react";
...
export const QuickSearchWindow: React.FC<QuickSearchWindowProps> = ({
  ...
  const lastThemeSyncAtRef = useRef(0);
  ...
  useEffect(() => {
    ...
    // 多个事件源（focus / visibilitychange / onFocusChanged / 轮询）在窗口
    // 刚显示时可能在短时间内集中触发多次，避免每次都发起 IPC 请求
    // 重新拉取主题，减少主线程竞争，让键盘输入更快地得到响应。
    const throttledSyncTheme = () => {
      const now = Date.now();
      if (now - lastThemeSyncAtRef.current < 250) return;
      lastThemeSyncAtRef.current = now;
      void syncTheme();
    };
    ...
    const onStorage = (event: StorageEvent) => {
      if (event.key === "searchis_config_v1" && event.newValue)
        void syncTheme();
    };
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      const element = document.querySelector(
        `[data-snippet-id="${selectedItem.id}"]`,
      );
      if (element) {
  ...
  const doPaste = async (item: SearchResultItem, keepOpen: boolean) => {
    ...
    setError(null);
    try {
      const outcome = await snippetsApi.executePaste(
        item.id,
        crypto.randomUUID(),
        !keepOpen,
      );
      setResults((current) =>
        current.map((result) =>
          result.id === outcome.snippet.id
            ? { ...result, usageCount: outcome.snippet.usageCount }
            : result,
        ),
      );
      if (!keepOpen) close();
  ...
  const doCopy = async (item: SearchResultItem) => {
    if (copyingRef.current) return;
    copyingRef.current = true;
    setError(null);
    try {
      const outcome = await snippetsApi.copy(
        item.id,
        crypto.randomUUID(),
        false,
      );
  ...
  const createFromQuery = async () => {
    ...
    try {
      closingRef.current = true;
      onCreateNewSnippet((await snippetsApi.prepareNew(raw)).normalizedKey);
    } catch (prepareError) {
      setError(
        (prepareError as { message?: string }).message ?? "无法生成 Key",
      );
    }
  };
  const moveSelection = (delta: number) =>
    setSelectedIndex((index) =>
      Math.max(0, Math.min(index + delta, results.length - 1)),
    );
  const onKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (!(event.ctrlKey || event.metaKey)) {
      if (event.key === "Enter" && selected) {
        event.preventDefault();
        void doPaste(selected, false);
      }
      return;
    }
    if (/^[1-9]$/.test(event.key)) {
      event.preventDefault();
      const item = results[Number(event.key) - 1];
      if (item) void doPaste(item, false);
      return;
    }
    if (event.key === "Enter" && selected) {
      event.preventDefault();
      void doPaste(selected, true);
      return;
    }
    if ((event.key === "e" || event.key === "E") && selected) {
      event.preventDefault();
      closingRef.current = true;
      onEditSnippet(selected);
      return;
    }
  ...
  const renderItem = (item: SearchResultItem) => {
    ...
    return (
      ...
      >
        <span
          className={`quick-key ${selectedIndex === index ? (item.pinned ? "quick-key-pinned-selected" : "quick-key-selected") : item.pinned ? "quick-key-pinned" : ""}`}
          aria-hidden
        >
          {item.pinned ? (
        ...
        <div className="min-w-0 flex-1 flex flex-col gap-1">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="truncate font-mono text-[11.5px] text-[color:var(--color-fg-secondary)]">
              {highlight(item.key, searchQuery)}
            </span>
            {item.sensitive && (
              <span className="quick-sensitive">
                <Lock className="size-3" />
                敏感
              </span>
            )}
            {item.usageCount > 0 && (
              <span className="quick-usage">使用 {item.usageCount} 次</span>
            )}
            {item.tags.slice(0, 2).map((tag) => (
              <span className="quick-tag" key={tag}>
                #{tag}
              </span>
            ))}
          </div>
        ...
        {index < 9 && (
          <CommandShortcut>
            <kbd className="raycast-kbd">Ctrl+{index + 1}</kbd>
          </CommandShortcut>
        )}
      </CommandItem>
    );
  ...
  return (
    <main className="quick-search-shell" aria-label="快速检索">
      <section
        className="quick-search-window"
        role="dialog"
        aria-modal="true"
        aria-label="快速检索窗口"
      >
        <Command
          ...
          value={selected?.id ?? ""}
          onValueChange={(id) =>
            setSelectedIndex(results.findIndex((item) => item.id === id))
          }
        >
          <CommandInput
            ref={inputRef}
            ...
            onKeyDown={onKeyDown}
            placeholder="搜索 Key、别名、标题、标签或正文…"
            aria-label="检索文本片段"
            autoFocus
          />
          {error && (
          ...
          <CommandList className="min-h-0 flex-1">
              ...
              {!loading && (
                <div className="flex flex-col items-center justify-center gap-3 px-4 text-center">
                  <span className="text-xs text-[color:var(--color-fg-muted)]">
                    {searchQuery ? "没有匹配的片段" : "暂无片段数据"}
                  </span>
                  {searchQuery && (
                    <button
                      type="button"
                      className="btn-primary inline-flex items-center justify-center gap-1.5 cursor-pointer transition-all px-2.5 py-1 text-xs rounded-md"
                      onClick={() => void createFromQuery()}
                    >
                      以该查询词为 Key 新建片段{" "}
                      <kbd className="raycast-kbd">Ctrl+N</kbd>
                    </button>
                  )}
                </div>
              )}
            ...
            {pinned.length > 0 && (
              <CommandGroup heading={`固定片段 · ${pinned.length}`}>
            ...
            {ordinary.length > 0 && (
              <CommandGroup
                heading={
                  pinned.length
                    ? `全部结果 · ${ordinary.length}`
                    : `匹配片段结果 · ${ordinary.length} 条`
                }
              >
        ...
        <footer className="quick-footer">
          <span className="flex items-center gap-2">
            ...
            <button
              type="button"
              className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[color:var(--color-fg-muted)] hover:text-[color:var(--color-fg)] hover:bg-[color:var(--color-surface-2)] transition-colors cursor-pointer"
              onClick={() => {
                closingRef.current = true;
                onOpenManager();
              }}
          ...
          <span className="flex items-center gap-2">
            <span>
              粘贴 <kbd className="raycast-kbd">↵</kbd>
            </span>
            <span>
              仅复制 <kbd className="raycast-kbd">Ctrl+↵</kbd>
            </span>
            <span>
              关闭 <kbd className="raycast-kbd">Esc</kbd>
            </span>
          </span>
        </footer>
      </section>
